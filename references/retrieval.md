...
| L2 | ZVEC 混合检索 | zvec SDK（可选装） | 关闭 | BM25 + dense 语义向量 + RRF 融合；同义/语义近似可召回 |

**BM25+FTS 与 ZVEC 的差别（初始化时告知用户）**：
- **BM25+FTS（L1，默认）**：词法匹配。查"沈青梧/剑胎/血月后第N天"这类**精确专名**命中率高；换同义词就查不到。零依赖（jieba 已有），索引小，初始化即可用。
- **ZVEC（L2，可选）**：词法+语义混合。问"主角现在处境如何"这种**语义近似**也能召回相关条目（dense 向量）；但需额外安装 zvec SDK（Windows 长路径可能阻 torch，见 embedding 注记），索引较大。
- **两者共有**：防剧透（source_chapter cutoff）+ 知情权（visibility）过滤，一致。
- **选择建议**：写长篇专名检索为主 → L1 已够；想要语义召回 / 防"换词查不到" → 开 L2。
...
| ⑤ 落库后 | `index.py` 增量 upsert 本章 | 正文块 + truth 变更入索引；正文改写按章删除重建 |
...
| `dense` | VECTOR | 中文 embedding 向量（本地 bge-small-zh-v1.5） |
...
- 唯一性保证：**拼音键 = 全局唯一**（同角色同键），中文名存 `entity_cn` 字段。
