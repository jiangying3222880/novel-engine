...
# Novel Engine v1.5.0 (小说创作引擎)

## v1.5.0 更新（2026-09-13 · 基线测试回填 + PMC suppression机制）

### 炼丹补充（job-0007~0047 · 2026-09-14）
...
- **T-001（剧情收获推演法）补强**：情节发展速度四档参数（job-0010）
- **T-002（人物四维塑模法）补强**：角色聚焦法+主角四忌+心理描写禁区+配角塑造四法（job-0011）
- **T-004（开篇钩子设计）补强**：第7种开局烘托法+第8种开局楔子伏笔法+人物不超过5个（job-0012）
- **T-010（高潮突转技法库）补强**：抑制+延宕憋压机制+期望/突发两种悬念+三种悬念认知结构+张弛/跌宕/蓄放区分（job-0013~0014）
- **T-005（爽点设计方法论）补强**：新增第七节"103经典桥段结构模式"（15个爽点结构模式）+ 第八节"三类补充爽点模式"（送菜流/优越感/养成体系节奏）（job-0015~0017）
- **T-012（代入感技法）补强**：社会脑/镜像神经元机制+蔡氏效应+泛性格策略+第一人称代入感优势与限制（job-0016~0018）
- **新增 T-013（对白口语技法）**：九层对白模型+口语硬规则9条+对白写作七步流程（job-0019）
- **新增 T-014（潜台词与角色声线）**：潜台词5种方式+角色声线四件套+六角色类型速查（job-0020）
- **新增 T-015（对白自检与题材适配）**：6类题材对白差异速查+十八条对白自检表（job-0021）
- **T-005（爽点设计方法论）补强**：新增第九节"虐主情节翻盘原则"（网游BOSS模型+三铁律）（job-0022）
- **T-002（人物四维塑模法）补强**：情节突显性格三法+开头人数控制（job-0022）
- **anti-ai-词级信号.md 补强（D级）**：AI写作禁令skill补充D级禁句式（job-0023）
- **T-014（潜台词与角色声线）补强**：新增第七节"遮名测试完整流程"+章后11条检查+常见错误5条（job-0026）
- **references/workflow-detail.md 补强**：写前读顺序（7步）+记忆分层（4层）+章后固定动作（10步）+交付物规范（job-0027）
- **T-001（剧情收获推演法）补强**：新增六b节"收获推演硬规则10条"+长线节奏密度+剧情诊断格式（job-0029）
- **genre/xuanhuan-chendong/生产流程.md 补强**：势力设定（job-0031）+ 番外设定（job-0031）
- **新增 references/商业立意与题材选择.md**：题材选择三禁区（武侠/科幻/推理新人禁区）+ 包装策略 + 风格即商业价值（job-0031）
- **T-005（爽点设计方法论）补强**：新增第十节"爽点稀缺性原理"（爽点三要素+前期爽点论+铺垫爽点法+三大忌）（job-0033）
- **references/workflow-detail.md 补强**：新人误区自检表6条（自嗨/细节/配角/速度/反馈/同质化）（job-0033）
- **T-005（爽点设计方法论）补强**：新增第十一节"爽点位置布置"（头尾型爽点3页法则+周期小结尾+高潮递进结构+高潮密度控制红线）（job-0034）
- **references/商业立意与题材选择.md 补强**：新增"起点读者画像"章节（十几岁男性+一人高法则+读者七愿望+商业禁忌）（job-0034）
- **新增 references/简介写法.md**：编辑教你写简介（简介三要素公式+成功/失败案例+自检表）（job-0035）
- **T-005（爽点设计方法论）补强**：新增第十二节"上架节奏法则"（前三万字法则+三阶段节奏+四个留存关键节点）（job-0036）
- **T-005（爽点设计方法论）补强**：新增第十三节"扮猪吃虎技法"（理由四分类+四要素+自检表）（job-0037）
- **新增 library/techniques/期待感技法.md（T-013）**：引字四法（伏笔提醒/连环伏笔/误导诱导/预给好处）+高手vs新手唯一区别（job-0038）
- **T-010（高潮突转技法库）补强**：新增误导诱导突转+预给好处原则（job-0038）
- **T-010（高潮突转技法库）补强**：新增承转启合技法（凤头猪肚麒麟尾对应+共同物承转法）（job-0039）
- **references/workflow-detail.md 补强**：新增打补丁操作表（时间/场景/人物切换漏洞补丁）（job-0039）
- **新增 references/读者吸引技法.md**：泡读者五法（共鸣/勾引/欺诈/投其所好/欲擒故纵）+第一印象六要素+看+变=与众不同（job-0040）
- **T-002（人物四维塑模法）补强**：新增主角性格决定论（性格→能力→行为模式）+配角炮灰设定格式（九线框架）（job-0041）
- **T-004（开篇钩子设计）补强**：新增小白文签约公式（10章结构）+关键数字表（300字/3000字/3章/3万字/30万字）（job-0042）
- **references/workflow-detail.md 补强**：新增商业写作硬规则表（16条规则）（job-0042）
- **新建 library/genres/仙侠修真/创作指南.md**：仙侠四大核心元素（等级/丹药法宝/恩怨情仇/获得失去）+14元素清单+元素组合拼接法+四流派+四卖点+仙侠vs玄幻区别+东方玄幻随心所欲（job-0043）
- **references/读者吸引技法.md 补强**：新增网游五要素（等级/装备/副本/PK/系统升级=网文五要素）+人民币玩家BOSS模型+副本写作时机（job-0044）
- **新增 library/techniques/穿越技法.md（T-015）**：穿越核心三问+高手vs新手区别+三个伏笔+穿越合理性技巧+现代知识碰撞玄幻规则（job-0045）
- **新建 library/genres/都市/创作指南.md**：都市12小类读者画像+代入门道+看+变创作法+都市vs仙侠区别+写作禁区+自检表（job-0046）
- **新建 library/genres/悬疑/创作指南.md**：悬疑四小类+突发式悬念+信息逐层释放+主角定位+悬疑vs都市神话区别+写作禁区+自检表（job-0047）
...
- **T-001~T-005 技法正式丹核化**：library/techniques/ 5个顶级技法加7字段头部（命题/层级/载体/证据/反例/状态/复审）

### PMC Suppression 机制（Step 4）
- **新增 references/target-focus.md**：目标焦点管理完整机制说明（PMC4266429实验+skill实现）
- **hooks.md 新增目标焦点表**：`g_xxx` 目标钩子（focal/suppressed/achieved），与信息钩子（`h_xxx`）并列
- **session-start.md §5 扩展**：规划时必须检查 focal goal 状态 + suppression 触发 + suppressed remention
- **机制依据**：PMC4266429（goal remention 714ms vs new goal 908ms, p<.001）

### Profiles层填充（Step 5）
- **04_profiles/ 建14个文件**：6平台（番茄长/短/起点/晋江/七猫/飞卢）+ 3题材（都市/修仙/悬疑）+ 3篇幅（长/中/短）+ _README
- **来源**：基线测试（B-001/B-002/B-003）+ library/platforms/_README.md
- **加载方式**：初始化时选定 platform + genre + length → 生成 novel-config.json profile字段

### 模型文档（Step 6）
- **新增 references/model-guide.md**：各阶段模型选择/提示策略/限制补偿速查
- **覆盖**：豆包（原生适配）/ Claude（外包首选）/ GPT-4（需治理）/ 其他模型
- **核心原则**：skill ≠ 任何单一模型；治理逻辑可在任何模型上运行，模型差异影响写作质量上限而非流程

### 短篇部分（节奏底层重构 · Hermes 侧同步 + 两测追补）
