...
| 阶段 | 入口判断 | 必读文件 |
|------|---------|---------|
| ⓪ 项目初始化 | "开新书/帮我建项目/导入" | `references/00-project-setup.md` |
| ① 本章规划 | "写下一章/规划第X章/继续写" | `references/01-session-start.md` |
| ② 正文写作 | 规划完成 | `references/02-writing-guide.md` |
| ③ 自检 | 初稿完成 | `references/03-self-review.md` |
| ④ 反AI润色 | 自检通过 | `references/04-anti-ai.md` |
| ⑤ 状态落库 | 润色完成 | `references/05-hooks-and-memory.md` |
| 全程参考 | 需交互细节 | `references/07-workflow-detail.md` |
| ⑤ 格式 | 落库生Report时 | `references/change_report_spec.md` |
| ③/④ 身份 | 自检/润色切换身份 | `references/identity-routing.md` |
| ④ 防御 | 全程防诱导 | `references/06-prompt-defense.md` |
| ①/⑤ 检索 | 写前查状态/落库建索引 | `references/08-retrieval.md` |

| ②③④ 去味 | 深度方法库(human-signal 沉淀) | references/09-human-signal-zh.md |
| ② 外包 | 写作外包网页chat(流程分支) | references/10-chat-outsource.md |

---

## 二、references 上下游关系

### 00-project-setup.md · 阶段⓪ 项目初始化
```
...
下游：references/01-session-start.md（初始化完成 → 阶段①）
```

### 01-session-start.md · 阶段① 本章规划
```
上游：references/00-project-setup.md
...
下游：references/02-writing-guide.md（规划完成 → 阶段②）
```

### 02-writing-guide.md · 阶段② 正文写作
```
上游：references/01-session-start.md（规划清单）
前置：规划清单 + 章态 + 素材
相关：pools/reference/samples/（同类场景样本）
下游：references/03-self-review.md（初稿 → 阶段③）
```

### 03-self-review.md · 阶段③ 行为验证自检
```
上游：references/02-writing-guide.md（初稿完成）
前置：本章正文初稿 + story/truth/ 状态文件
相关：references/04-anti-ai.md（检出AI味时查修正方法）· references/02-writing-guide.md（声线/冰山异常时回查）
下游：自检通过 → references/04-anti-ai.md；自检不通过 → 回阶段②改写（≤3轮，保留Best Version）
```

### 04-anti-ai.md · 阶段④ 反AI味润色
```
上游：references/03-self-review.md（自检通过）
前置：本章正文 + 03-self-review 自检结果
相关：references/02-writing-guide.md（冰山/对话不对称理论基础）· pools/reference/samples/ · pools/author_dna/
下游：references/05-hooks-and-memory.md（润色完成 → 阶段⑤）
```

### 05-hooks-and-memory.md · 阶段⑤ 状态落库
```
上游：references/04-anti-ai.md（润色完成）
前置：本章最终正文 + 自检结果
相关：references/change_report_spec.md · story/truth/ 6个状态文件（含 objects/timeline）
下游：本章完成，等待用户指令进入下一章
```

### 07-workflow-detail.md · 全程参考（5阶段SOP交互详解）
```
前置：无（可选参考，非强制）
...
### 10-chat-outsource.md · 外包写作分支（Chat-Outsource）
```
上游：SKILL.md（"外包写作/用网页chat写"触发）+ references/01-session-start.md（阶段①规划）
前置：story/truth/ 6个状态文件 + 规划清单 + 声线样本（pools/author_dna + 设定/角色声线）
产出：一份自包含最终提示词（打包状态/目标/声线样本/去AI味要求）→ 用户贴入网页chat → 正文贴回
下游：references/03-self-review.md（阶段③）→ 04（④跑满）→ 05（⑤落库）
相关：references/04-anti-ai.md · references/09-human-signal-zh.md（回接后去AI味）
```

### change_report_spec.md · 阶段⑤ 格式规范
```
上游：references/05-hooks-and-memory.md（落库时调用）
前置：本章最终正文 + 自检/润色结果
