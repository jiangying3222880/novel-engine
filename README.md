...
| 记忆漂移 / 前后矛盾     | story/truth/ 6 文件状态中心（+objects/timeline），Change Report 驱动更新  |
...
* `story/truth/` — 6 个状态文件（characters/world/hooks/relationships/objects/timeline，强制加载）

* `story/meta/` — 章节元数据集中索引（index.md 进度真相源 + chapter\_XXX.md）

* `story/logs/` — 每章流程日志

* `叙事总览/` — Obsidian 纯展示层总览

* `pools/` — 5 个素材池（按需加载，Agent 自动拆解生成）
...
| **v1.2.0** | 长文一致性 / 风格漂移 | truth/ 四层状态中心 + Change Report + pools/ 素材池 + 技能项目分层 |
